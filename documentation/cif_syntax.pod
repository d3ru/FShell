...
=head3 Important note about TUids

Although it is acceptable (if you wish) to pass a casted TUid instead of a TUint to an AppendUintL() API, the same is not true if multiple values are accepted. In that case your member variable I<must> be of type RArray<TUint> and I<not> of type RArray<TUid>. The same goes for any other type like TUid that is more than just a typedef for TUint or TInt, even if it is still 32 bits and looks like it should be bitwise compatible with TUint or TInt. This is because RArray<TUint> and RArray<TInt> are template specialisations with a different internal structure to an RArray of any other 32-bit datatype. In other words very few things can be safely cast to or from a RArray<TUint>. This is a peculiarity of the OS and not strictly an fshell bug.
