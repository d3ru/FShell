...
=item FSHELL_SMP_SUPPORT

(Experimental) Used to indicate that SIS files created by this platform should include armv5smp binaries. All the appropriate SMP binaries can be build by using the bld.inf located in \fshell\build\common\drivers\bld.inf.
