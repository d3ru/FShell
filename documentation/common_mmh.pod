...
Used to indicate that SIS files created by this platform should include armv5smp binaries as well as the usual armv5 ones. All the appropriate SMP binaries can be build by using the bld.inf located in \fshell\build\common\drivers\bld.inf.
