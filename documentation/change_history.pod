...
=head2 Release 006 **Not yet released**
...
Added mcl platform located at \fshell\build\s60\mcl. This platform supports building fshell for full gui and minigui at the same time. To build, do:

    M:\fshell\build\s60\mcl>sbs -c armv5_urel
    M:\fshell\build\s60\mcl>sbs -c armv5_urel.fshellminigui

Then to build a full gui rom with fshell included do:

    imaker <device> core BLDROBY=\epoc32\rom\include\fshell.oby
...
Added L<proxyserver|commands::proxyserver>, L<traceswitch|commands::traceswitch>, L<subst|commands::subst>, L<startapp|commands::startapp> and L<memmap|commands::memmap> commands.
...
Changed C<match --count> to return just an integer (rather than "Count = 2") so as to be more useful in scripts.
