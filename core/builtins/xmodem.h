...
protected:
	...
	void ReceiveToNullL();
	void SendBlockL(const TDesC8& aBlock);
	TPtrC8 ReceiveBlockL(TBool aIsFirstBlock, TBool& aIsFinalBlock);
	void DoReceiveBlockL();
	void SendTerminateL();
	void WaitForSyncL();
	TSyncResult SendSyncL();
	TUint8 ReceiveByteL(TInt aTimeout, TBool* aTimeoutOccurred = NULL);
	void ReceiveWithTimeoutL(TDes8& aBuf, TInt aTimeout, TBool* aTimeoutOccurred = NULL);
	void DoReceiveWithTimeoutL(TDes8& aBuf, TInt aTimeout, TBool* aTimeoutOccurred = NULL);
	void PurgeInputL();
